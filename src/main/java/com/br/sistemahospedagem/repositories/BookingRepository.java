...
public interface BookingRepository extends JpaRepository<BookingModel, Long>{
    ...
    Optional<BookingModel> findBookingByRoom(RoomSchema roomId);
    Optional<BookingModel> findBookingById(Long id);
    ...
    List<BookingModel> findBookingsByDates(@Param("checkIn") LocalDate checkIn, @Param("checkOut") LocalDate checkOut);
    @Query(value = "SELECT * FROM bookings_table WHERE room_id = ?1 ORDER BY booking_id DESC LIMIT 1", nativeQuery = true)
    BookingModel findLatestBookingByRoomId(int roomId);
} 
